...
<main
	class={clsx(
		'w-full h-full relative bg-gradient-to-b from-blue-900 to-blue-500 flex flex-col items-center justify-center overflow-hidden '
	)}
>
	<div class={clsx('w-full overflow-auto p-4')}>
		<div class={clsx('flex flex-col justify-center items-center gap-8 ')}>
			<div class={clsx('text-blue-950 relative z-10')}>
				<SvelteLogo size={250} />
			</div>
			<div
				class={clsx('flex flex-col gap-2 text-blue-50 max-w-2xl relative z-10 nightwind-prevent')}
			>
				<h1 class={clsx('text-2xl')}>Welcome to my Svelte playground!</h1>
				<h2>
					To start using the application you need to login or sign-up. <br /> Don't worry during the
					sign-up process you will only be asked to provide a username and a password, no personal information
					is required.
				</h2>
